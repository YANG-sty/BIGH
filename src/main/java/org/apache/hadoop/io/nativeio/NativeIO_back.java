...
public class NativeIO_back {
    ...
    /* Wait upon writeout of all pages
       ...
       write.  */
        ...
        private static final Logger LOG = LoggerFactory.getLogger(NativeIO_back.class);
        ...
        public static class CacheManipulator {
            ...
            public long getMemlockLimit() {
                return NativeIO_back.getMemlockLimit();
            }

            public long getOperatingSystemPageSize() {
                return NativeIO_back.getOperatingSystemPageSize();
            }

            public void posixFadviseIfPossible(String identifier,
                                               FileDescriptor fd, long offset, long len, int flags)
                    throws NativeIOException {
                NativeIO_back.POSIX.posixFadviseIfPossible(identifier, fd, offset,
                        len, flags);
            }

            public boolean verifyCanMlock() {
                return NativeIO_back.isAvailable();
            }
        }
    ...
    public static FileDescriptor getShareDeleteFileDescriptor(
        ...
        if (!Shell.WINDOWS) {
            ...
            // given offset.
            //
            FileDescriptor fd = NativeIO_back.Windows.createFile(
                    f.getAbsolutePath(),
                    NativeIO_back.Windows.GENERIC_READ,
                    NativeIO_back.Windows.FILE_SHARE_READ |
                            NativeIO_back.Windows.FILE_SHARE_WRITE |
                            NativeIO_back.Windows.FILE_SHARE_DELETE,
                    NativeIO_back.Windows.OPEN_EXISTING);
            if (seekOffset > 0)
                NativeIO_back.Windows.setFilePointer(fd, seekOffset, NativeIO_back.Windows.FILE_BEGIN);
            return fd;
        }
    }
    ...
    public static FileOutputStream getCreateForWriteFileOutputStream(File f, int permissions)
        ...
        if (!Shell.WINDOWS) {
            ...
            try {
                FileDescriptor fd = NativeIO_back.POSIX.open(f.getAbsolutePath(),
                        NativeIO_back.POSIX.O_WRONLY | NativeIO_back.POSIX.O_CREAT
                                | NativeIO_back.POSIX.O_EXCL, permissions);
            ...
            try {
                FileDescriptor fd = NativeIO_back.Windows.createFile(f.getCanonicalPath(),
                        NativeIO_back.Windows.GENERIC_WRITE,
                        NativeIO_back.Windows.FILE_SHARE_DELETE
                                | NativeIO_back.Windows.FILE_SHARE_READ
                                | NativeIO_back.Windows.FILE_SHARE_WRITE,
                        NativeIO_back.Windows.CREATE_NEW);
                NativeIO_back.POSIX.chmod(f.getCanonicalPath(), permissions);
                return new FileOutputStream(fd);
            } catch (NativeIOException nioe) {
